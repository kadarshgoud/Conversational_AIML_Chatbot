...
namespace AIMLBot.Utils
{
    ...
    public class AIMLLoader
    {
        ...
        public void loadAIML()
        {
            ...
            this.loadAIML(this.bot.PathToCustomAIML);
        }
        ...
        public void loadAIML(string path)
        {
            if (Directory.Exists(path))
            {
                ...
                {
                    if (directories[i].EndsWith(".aiml"))
                    {
                ...
                {
                    // process the AIML
                    this.bot.writeToLog("Starting to process AIML files found in the directory " + dicrectory);

                    string[] fileEntries = Directory.GetFiles(dicrectory, "*.aiml");
                    ...
                    {
                        throw new FileNotFoundException("Could not find any .aiml files in the specified directory (" + dicrectory + "). Please make sure that your aiml file end in a lowercase aiml extension, for example - myFile.aiml is valid but myFile.AIML is not.");
                    }
                }
            }
            else
